...
class Config:
    ...
    def read_json_micro_manager(self):
        ...
        self._load_balancing = _get_optional(simulation_params, "load_balancing", False)
        if self._load_balancing:
            self._logger.log_info_rank_zero(
                "Micro Manager will dynamically balance micro simulations."
            )
            self._write_data_names.append("rank_of_sim")
            if self._adaptivity and self._adaptivity_type != "global":
                raise ConfigError(
        ...
        if self._load_balancing:
            ...
            self._load_balancing_partitioning = _get_optional(
                load_balancing_settings,
                "partitioning",
                self._load_balancing_partitioning,
                "load_balancing_settings",
            )

            self._load_balancing_type = _get_optional(
                load_balancing_settings, "type", "time", "load_balancing_settings"
            )
            if self._load_balancing_type not in ("time", "active"):
                raise ConfigError(
                    "load_balancing_settings.type must be either 'time' or 'active'. "
                    f"Got: '{self._load_balancing_type}'"
                )
            self._logger.log_info_rank_zero(
            ...
            if self._load_balancing_type == "active":
                self._load_balancing_threshold = _get_optional(
                    load_balancing_settings, "threshold", 0
                )
                self._logger.log_info_rank_zero(
                    "Load balancing threshold: " + str(self._load_balancing_threshold)
                )

                self._load_balancing_balance_inactive_sims = _get_optional(
                    load_balancing_settings, "balance_inactive_sims", False
                )
                if self._load_balancing_balance_inactive_sims:
                    self._logger.log_info_rank_zero(
                ...
                if "threshold" in load_balancing_settings:
                    self._logger.log_info_rank_zero(
                        'Load balancing is not using active simulation balancing. Field "threshold" will be ignored.'
                    )
                if "balance_inactive_sims" in load_balancing_settings:
                    self._logger.log_info_rank_zero(
