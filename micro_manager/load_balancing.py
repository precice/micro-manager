...
class LoadBalancer:
    def __init__(self,
        ...
        self._partition_impl = self.get_partition_impl(config.get_load_balancing_partitioning())

        if adaptivity_controller is not None and type(adaptivity_controller).__name__ != "GlobalAdaptivityCalculator":
            raise NotImplementedError("Adaptivity must be GlobalAdaptivity for Load Balancing")
    ...
    def get_partition_impl(self, name):
        if name == "lpt": return self.partition_lpt
        else: return self.partition_dummy

    def partition_lpt(self, n_parts, current_partitioning):
        sorted_workload_indices = np.argsort(self._balance_metric_global)[::-1] # descending
    ...
    def partition_dummy(self, n_parts, current_partitioning):
        ...
        return np.zeros(self._global_number_of_sims, dtype=np.int32), workload_per_partition

    # ==================
    #      HELPERS
    # ==================
    def _redistribute(self):
        self._precice_participant.start_profiling_section("micro_manager.solve.load_balancing.init")
        ...
        target_partitioning, work_loads = self._partition_impl(self._comm.size, current_partitioning)
        #self._precice_participant.stop_last_profiling_section()
        send_map, recv_map = self._get_communication_maps(current_partitioning, target_partitioning)
        inactive_gids = self._get_global_inactive_gids()
        self._precice_participant.stop_last_profiling_section()

        self._precice_participant.start_profiling_section("micro_manager.solve.load_balancing.comm")
    ...
    def _get_communication_maps(
        ...
        for gid, target_rank in enumerate(target_partitioning):
            if current_partitioning[gid] == target_rank: continue
    ...
    def _get_global_active_gids(self):
        ...
        if self._adaptivity_controller is not None:
            active_gid_arr = [
                self._global_ids[i] for i in self._adaptivity_controller.get_active_sim_local_ids()
            ]
        else:
    ...
    def _get_global_inactive_gids(self):
        ...
        global_inactive_gids = global_active_gids.difference(set(np.arange(self._global_number_of_sims)))
    ...
    def _exchange_sims(self, send_map, recv_map, inactive_map={}):
        ...
        for gid, send_rank in send_map.items():
            ...
            req = self._comm.isend(
                send_data, dest=send_rank, tag=tag
            )
...
class CountLB(LoadBalancer):
    ...
    def __init__(self,
        ...
        super().__init__(precice_participant, model_manager, adaptivity_controller, state_loader, state_setter, log, config, sim_list, global_ids, comm, rank)
