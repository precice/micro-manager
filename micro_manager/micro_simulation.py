...
class MicroSimulationList:
    def __init__(self, exe, num_ranks, size, fill_value=None):
        self._exe = exe
        self._num_ranks = num_ranks
    ...
    def construct_instances(self, indices, gids, sim_cls_wrapper):
        # idx to construct
        # gid of respective idx
        assert len(indices) == len(gids)

        multi_rank = self._num_ranks > 1
        for pos in range(len(indices)):
            idx, gid = indices[pos], gids[pos]
            self._data[idx] = sim_cls_wrapper(gid, late_init=multi_rank)

        if not multi_rank: return

        f_gen_instances = self._exe.submit(
            MicroSimulationList.construct_instances_local,
            # args
            gids=gids,
            sim_cls=sim_cls_wrapper.backend_cls,
            num_ranks=self._num_ranks,
            # execution params
            resource_dict={"cores": self._num_ranks},
        )

        for rank, sim_states in f_gen_instances.result():
            for pos in range(len(indices)):
                idx = indices[pos]
                self._data[idx]._states[rank] = sim_states[pos]

    @staticmethod
    def construct_instances_local(gids, sim_cls, num_ranks):
        from mpi4py import MPI
        ...
        states = [None] * len(gids)
        for idx, gid in enumerate(gids):
            states[idx] = sim_cls(gid).get_state()
