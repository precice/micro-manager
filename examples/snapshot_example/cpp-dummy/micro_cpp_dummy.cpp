...
// Postprocessing
py::dict MicroSimulation::postprocessing(py::dict micro_data) {
    double micro_scalar_data = micro_data["micro-scalar-data"].cast<double>();
    double _micro_scalar_data = micro_scalar_data + 10.;


    micro_data["micro-scalar-data"] = _micro_scalar_data;
...
PYBIND11_MODULE(micro_dummy, m) {
    ...
    py::class_<MicroSimulation>(m, "MicroSimulation")
        ...
        .def("postprocessing", &MicroSimulation::postprocessing)
